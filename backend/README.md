...
## Форматированный текст в админке

Поля `body` (Местное содержание, Новости) и `description` (Закупки) редактируются через WYSIWYG-редактор (Quill, подключён с CDN — см. `content/widgets.py`, без отдельного pip-пакета вроде django-ckeditor, чтобы не зависеть от его совместимости с версией Django). Доступно: заголовки, **жирный**/*курсив*/подчёркивание, списки, ссылки, **картинки**.

Поле в базе остаётся обычным `models.TextField` — просто хранит HTML вместо голого текста. На сайте эти поля рендерятся через `{{ entry.body|safe }}` (не экранируются) — осознанно, потому что писать в них может только аутентифицированный суперюзер через `/admin/`, обычной формы на сайте для этого нет. Если когда-нибудь появится поле, куда сможет писать не-админ (форма на сайте и т.п.) — для него `|safe` использовать нельзя, нужен обычный текст без HTML.

**Картинки в редакторе** загружаются по-настоящему на сервер (`POST /admin/quill-upload/`, `content.views.quill_image_upload`), а не встраиваются как base64 в само текстовое поле — иначе TextField и HTML-страница раздувались бы с каждой вставленной картинкой. Эндпоинт защищён `@staff_member_required` (та же проверка, что у `/admin/`) — не публичный, иначе кто угодно мог бы заливать файлы на сервер. Файлы сохраняются в `media/quill_uploads/` (не в git, см. `.gitignore`), ограничение — 8МБ и только `image/*`.

## Страница на каждую запись «Местное содержание»

`GET /local-content/<id>/` (`content.views.local_content_detail`, шаблон `local_content_detail.html`) — заголовок в списке (`/local-content/`) теперь ссылка на эту страницу, плюс превью текста через `{{ entry.body|truncatewords_html:25 }}` (безопасно обрезает HTML, не ломая теги) и кнопка «Читать полностью →». URL по числовому `pk`, не по slug — пока так проще, слаг можно добавить позже, если понадобятся более читаемые ссылки.
